...
opacity node/.style={anchor=north west,font=\FontSize{100pt}\bfseries\titlefont,text=ColorB!25}
}
\begin{titlepage}
...
%%封面图%%width=\paperwidth,
\node[inner sep=0mm,anchor=center](CI)at(G){\ifdefvoid{\@coverimage}{}{\includegraphics[height=\paperheight/2+5.25cm]{\@coverimage}}};
%%标题%%
\node[font=\FontSize{55pt}\bfseries\titlefont,coverup node](T)at(current page text area.north west){\@title};
%%英文标题%%
\node[font=\FontSize{20pt}\bfseries\titlefont,coverup node](ET)at([yshift=-0.5cm]T.south west){\ifdefvoid{\@englishtitle}{}{\@englishtitle}};
%%副标题%%
\node[font=\FontSize{30pt}\bfseries\titlefont,coverup node](ST)at([yshift=-0.5cm]ET.south west){\ifdefvoid{\@subtitle}{}{\@subtitle}};
%%英文副标题%%
\node[font=\FontSize{15pt}\bfseries\titlefont,coverup node](EST)at([yshift=-0.5cm]ST.south west){\ifdefvoid{\@englishsubtitle}{}{\@englishsubtitle}};
%%作者%%
\node[font=\FontSize{15pt}\bfseries\titlefont,coverdown node](A)at([yshift=4.0cm]current page text area.west){主编\quad\@author};
%%出版社%%
\node[font=\bfseries\titlefont,coverdown node](PL)at(current page text area.south west){\ifdefvoid{\@presslogo}{}{\includegraphics[width=0.75cm]{\@presslogo}}};
...
%%背景2%%
\draw[ColorA!80,fill=ColorA!80](E)rectangle(C);
%%封底图%%width=\paperwidth,
\node[inner sep=0mm,anchor=center](BI)at(G){\ifdefvoid{\@backimage}{}{\scalebox{1}[1]{\includegraphics[height=\paperheight/2+5.25cm]{\@backimage}}}};
%%条形码%%
\IfValueTF{#2}{
...
%%说明文字%%
\node[font=\FontSize{12pt}\bfseries\titlefont,backup node](BT)at(current page text area.north east){\hspace*{2.0em}#1};
%%书籍系列%%
\node[font=\FontSize{15pt}\bfseries\titlefont,backdown node](S)at([yshift=4.0cm]current page text area.west){\ifdefvoid{\@series}{}{\@series}};
%%书籍系列%%
\node[font=\FontSize{15pt}\bfseries\titlefont,backdown node](V)at([yshift=-0.5cm]S.south west){\ifdefvoid{\@version}{}{第{\@version}版}};
\end{tikzpicture}
\end{titlepage}}
...
\tikzset{spine title/.style={font=\FontSize{20pt}\bfseries\titlefont,text=white,text width=1.0cm,align=center},
spine subtitle/.style={font=\FontSize{15pt}\bfseries\titlefont,text=white,text width=1.0cm,align=center},
spine englishtitle/.style={font=\FontSize{20pt}\bfseries\titlefont,text=ColorA,inner sep=0mm}}
\begin{titlepage}
\begin{tikzpicture}[remember picture,overlay]
