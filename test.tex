% !TEX program = bibtex
% !Mode:: "TeX:UTF-8"
\documentclass[color=orange,openany]{textbook-cn}
...
%%设置索引样式%%
\indexsetup{level=\chapter*,envlevel=\chapter,toclevel=chapter,envtoclevel=chapter}
\makeindex[name=noun,title=名词索引,columns=3,intoc,columnsep=2.0em,options=-s indexstyle.ist]
\makeindex[name=video,title=视频索引,columnsep=2.0em,intoc,options=-s indexstyle.ist]

%%设置参考文献样式%%
\bibsetup{level=\chapter*,envlevel=\chapter,toclevel=chapter,envtoclevel=chapter,intoc}
...
\lipsum
\begin{figure}[htbp]
\centering\includegraphics[width=2\linewidth/5]{example-image-a}
\caption{极光}
\end{figure}
